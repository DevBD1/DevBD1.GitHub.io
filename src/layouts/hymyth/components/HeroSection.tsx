...
import { ChevronDown, Sword, Scroll, MapPin } from 'lucide-react';
    ...
    <div ref={ref} className="relative h-screen w-full overflow-hidden flex items-center justify-center">
      ...
      >
        ...
        <motion.h1
          ...
          className="font-cinzel font-black text-6xl md:text-8xl lg:text-9xl text-white mb-4 leading-[0.9] drop-shadow-[6px_6px_0_rgba(0,0,0,1)] tracking-tight relative"
        >
          <span className="relative z-10">{profile.name}</span>
          {/* Decorative duplicate for deeper shadow/glitch effect */}
          <span className="absolute inset-0 text-slate-900 -z-10 translate-y-2 translate-x-2 blur-[1px]">{profile.name}</span>
        </motion.h1>
        ...
        >
            <p className="text-slate-300 text-2xl md:text-4xl font-vt323 leading-relaxed drop-shadow-lg bg-black/20 px-6 py-2 rounded-lg border border-white/5 backdrop-blur-sm">
            <span className="text-myth-gold">Class:</span> {profile.title}
            </p>
            <div className="flex items-center gap-2 text-slate-400 font-cinzel text-lg">
                <MapPin size={18} />
                {profile.location}
            </div>
        </motion.div>
        ...
        >
             ...
             <a 
                href="#projects"
                className="group relative inline-flex items-center gap-4 px-12 py-5 bg-green-900/80 text-white font-vt323 text-3xl uppercase tracking-wider transition-all shadow-voxel-btn hover:shadow-voxel-btn-hover active:shadow-voxel-btn-active active:translate-y-1 hover:bg-green-800 border-b-4 border-r-4 border-black/40 border-t border-l border-white/10 rounded-sm backdrop-blur-sm"
             >
                <Sword className="w-8 h-8 group-hover:-rotate-45 transition-transform text-green-200" />
                <span className="drop-shadow-md">View Artifacts</span>
             </a>
             ...
             >
                <Scroll className="w-8 h-8 group-hover:scale-110 transition-transform text-slate-400" />
                <span className="drop-shadow-md">Read Chronicles</span>
             </a>
        </motion.div>
      </motion.div>
